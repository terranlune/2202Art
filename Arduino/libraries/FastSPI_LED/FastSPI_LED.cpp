...
// specific interrupt function through a pointer adds approximately 1.3Âµs over the if/else blocks 
// below per cycle.  That doesn't sound like a lot, though, right?  Wrong.  For the HL-1606, with
// 40 leds, you may want to push up to 125,000 cycles/second.  1.3Âµs extra per cycle means 162.5
// milliseconds.  Put a different way, you just cost yourself 16.25% of your cpu time.  This is on 
// a 16Mhz processor.  On an 8Mhz processor, the hit is even more severe, percentage wise.  1.3Âµs
// is also, for another point of data, only 20 clocks on a 20Mhz processor.  Every extra register push/pop
// that needs to be added to the function is 4 cycles (2 for push, 2 for pop) - or .25Âµs.  To call through
...
void CFastSPI_LED::setup_hardware_spi(void) {
  ...
  if(USE_TIMER) { 
    ...
    // This gives us the time for 10 rounds in Âµs
